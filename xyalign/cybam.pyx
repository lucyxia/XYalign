...
class BamFile():
	...
	def analyze_bam_fetch(self, chrom, window_size, target_file=None):
		...
		cdef int last_window_len
		cdef int total_read_length
		cdef int ref_start
		cdef int query_length
		cdef int ref_end
		cdef int window_id = 0
		cdef int start = 0
		cdef int end
				...
				for read in samfile.fetch(chrom, start, end):
					if read.is_secondary is False:
						if read.is_supplementary is False:
							...
							ref_end = ref_start + query_length
							if ref_start < start:
								total_read_length += (ref_end - start) + 1
							elif ref_end > end:
								total_read_length += (end - ref_start)
							else:
								total_read_length += query_length
